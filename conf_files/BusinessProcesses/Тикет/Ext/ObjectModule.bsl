...
// 	Отказ - Булево - 
// 
Процедура ОбработатьЗаявкуПриСозданииЗадач(ТочкаМаршрутаБизнесПроцесса, ФормируемыеЗадачи, Отказ)
...
Процедура ОбработатьЗаявкуПриВыполнении(ТочкаМаршрутаБизнесПроцесса, Задача, Отказ)
	
	если НомерИтерации = 1 тогда
		РезультатВыполнения = РезультатВыполненияПерваяЗапись(Задача);
	конецЕсли;	
...
Процедура ПроверитьПриСозданииЗадач(ТочкаМаршрутаБизнесПроцесса, ФормируемыеЗадачи, Отказ)
	...
	Для каждого Задача Из ФормируемыеЗадачи Цикл
		...
		Задача.Наименование = НаименованиеЗадачиДляПроверки(Задача.Выполнена);
...
Процедура ОформитьПриСозданииЗадач(ТочкаМаршрутаБизнесПроцесса, ФормируемыеЗадачи, Отказ)
		
	Для каждого Задача Из ФормируемыеЗадачи Цикл
		
		Задача.Автор = Автор;
		Задача.РольИсполнителя = Справочники.РолиИсполнителей.Оператор;
		Задача.Наименование = НаименованиеЗадачиДляОформления();
		Задача.СрокИсполнения = КонецМесяца(СрокПроверки);
		Задача.Важность = Важность;
		Задача.Предмет = Предмет;
...
Процедура ОформитьПриВыполнении(ТочкаМаршрутаБизнесПроцесса, Задача, Отказ)
	Завершен = Истина;
КонецПроцедуры
	...
	Попытка
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			...
			ЗадачаОбъект.Наименование = 
				?(ЗадачаОбъект.ТочкаМаршрута = БизнесПроцессы.Задание.ТочкиМаршрута.Выполнить, 
				НаименованиеЗадачиДляВыполнения(), НаименованиеЗадачиДляПроверки(ЗадачаОбъект.Выполнена));
...
Функция НаименованиеЗадачиДляПроверки(ЗадачаВыполнена)
	
	
	
	СтрокаФормат = ?(Выполнено, НСтр("ru = ' Подтвердить выполнение задачи'") + ": %1",
								НСтр("ru = ' Подтвердить ОТМЕНУ задачи'") + ": %1");

	
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СтрокаФормат, Наименование);
	
КонецФункции

Функция НаименованиеЗадачиДляОформления()
	
	Возврат БизнесПроцессы.Тикет.ТочкиМаршрута.Оформить.НаименованиеЗадачи + ": " + Наименование;
...
Функция РезультатВыполненияПерваяЗапись(Знач ЗадачаСсылка)
	
	
	СтрокаФормат = "%1, %2 " + НСтр("ru = ' поставил задачу'") + 
					"
					|";
...
Процедура ЗаявкаПодтвержденаПроверкаУсловия(ТочкаМаршрутаБизнесПроцесса, Результат)
	Результат = Подтверждено;
КонецПроцедуры

Процедура ЗаявкаВыполненаПроверкаУсловия(ТочкаМаршрутаБизнесПроцесса, Результат)
