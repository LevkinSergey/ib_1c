...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Объект.Ссылка.Пустая() Тогда
		УстановитьЗначенияПоУмолчаниюДляНового();
		ИнициализацияФормы();
	иначе
		ЗаполнитьПризнакиНаличияУдаленногоДоступа();
	КонецЕсли;
...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	Содержание = ТекущийОбъект.Содержание.Получить();
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	ТекущийОбъект.Содержание = Новый ХранилищеЗначения(Содержание);
...
//Rr_{
&НаСервере
Процедура УстановитьЗначенияПоУмолчаниюДляНового()
	...
	выбНастройки = запрос.Выполнить().выбрать();
	
	пока выбНастройки.Следующий() цикл
		если выбНастройки.настройка = ПланыВидовХарактеристик.НастройкиПользователей.ИсполнительТикета тогда
			объект.Исполнитель = выбНастройки.значение;
		иначеЕсли выбНастройки.настройка = ПланыВидовХарактеристик.НастройкиПользователей.КоличествоДнейНаЗакрытиеТикета тогда
			объект.СрокИсполнения = КонецДня(ТекущаяДатаСеанса() + выбНастройки.значение * 86400);
		иначеЕсли выбНастройки.настройка = ПланыВидовХарактеристик.НастройкиПользователей.ОбязательнаяПроверкаТикетов тогда
			объект.НаПроверке = выбНастройки.значение;
		КонецЕсли;	
	КонецЦикла;				
	
	если объект.НаПроверке тогда
		объект.Проверяющий = Справочники.РолиИсполнителей.ОтветственныйЗаКонтрольИсполнения;
		Объект.СрокПроверки = объект.СрокИсполнения + 86400;
	конецЕсли;
	
КонецПроцедуры	

Процедура ЗаполнитьПризнакиНаличияУдаленногоДоступа()
	
	для каждого стрПодразделений из объект.ПодразделенияПартнеров цикл
		стрПодразделений.естьУдаленныйДоступ = Справочники.УдаленныйДоступ.Выбрать(, стрПодразделений.Подразделение).Следующий();
	КонецЦикла;	
	
КонецПроцедуры	

&НаКлиенте
Процедура ПодразделенияПартнеровПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
